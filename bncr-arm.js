...
const CONFIG={name:'Bncr-ARM',version:'3.1.0',port:PORT,admin:{username:'admin',password:'admin123'}};
const utils={
  ...
  formatTime:ms=>{
    ...
    if(d>0)return d+'天'+(h%24)+'小时';if(h>0)return h+'小时'+(m%60)+'分';if(m>0)return m+'分'+(s%60)+'秒';return s+'秒';
  },
  formatBytes:b=>{if(b===0)return'0 B';const k=1024,sizes=['B','KB','MB','GB'],i=Math.floor(Math.log(b)/Math.log(k));return parseFloat((b/Math.pow(k,i)).toFixed(2))+' '+sizes[i];}
};
...
class Logger{
  constructor(){this.logFile=path.join(dirs.logs,utils.today()+'.log');this.logs=[]}
  log(level,type,msg){const line='['+utils.now()+'] ['+level+'] ['+type+']: '+msg;console.log(line);this.logs.unshift({time:utils.now(),level,type,msg});if(this.logs.length>500)this.logs=this.logs.slice(0,500);try{fs.appendFileSync(this.logFile,line+'\n')}catch(e){}}
...
class Database{
  constructor(){this.dbFile=path.join(dirs.db,'data.json');this.cfgFile=path.join(dirs.config,'system.json');this.data=this.loadDb();this.cfg=this.loadCfg();this.sessions=new Map();this.msgs=[];this.startTime=Date.now();this.init()}
  ...
  defaultCfg(){return{adapters:{tgbot:{name:'tgbot',enabled:false,type:'telegram',desc:'Telegram Bot'},qqbot:{name:'qqbot',enabled:false,type:'qq',desc:'QQ Bot'},wxbot:{name:'wxbot',enabled:false,type:'wechat',desc:'微信 Bot'},system:{name:'system',enabled:true,type:'system',desc:'系统适配器'}},plugins:{},settings:{logLevel:'info',maxMsgs:1000}}}
  init(){
    const plugins={
      'system.js':'module.exports=async(msg,db)=>{const t=msg.text?.trim();if(!t)return null;if(t==="ping")return"Pong!";if(t==="time")return new Date().toLocaleString("zh-CN");if(t==="版本")return"Bncr-ARM v3.1.0";const g=t.match(/^get\\s+(\\S+)\\s+(\\S+)$/);if(g)return JSON.stringify(db.get(g[1],g[2]));const s=t.match(/^set\\s+(\\S+)\\s+(\\S+)\\s+(.*)$/);if(s){db.set(s[1],s[2],s[3]);return"已设置";}return null;}',
    ...
    for(const[f,c]of Object.entries(plugins)){const p=path.join(dirs.plugins,f);if(!fs.existsSync(p)){fs.writeFileSync(p,c);const name=f.replace('.js','');this.cfg.plugins[name]={enabled:true,desc:name+'插件'}}}
    this.save()
  }
...
class WebServer{
  ...
  route(req,res){
    ...
    if(!token||!this.db.check(token)){if(url.startsWith('/api/'))return this.json(res,401,{error:'未授权'});return this.serveLogin(res)}
  ...
  doLogin(req,res){const{username,password}=req.body||{};if(username===CONFIG.admin.username&&password===CONFIG.admin.password){const token=this.db.newSession();return this.json(res,200,{ok:true,token})}return this.json(res,401,{error:'用户名或密码错误'})}
...
<form id="loginForm"><div class="input-group"><label>用户名</label><input type="text" id="username" placeholder="admin" required autocomplete="username"></div>
<div class="input-group"><label>密码</label><input type="password" id="password" placeholder="admin123" required autocomplete="current-password"></div>
...
document.getElementById('loginForm').addEventListener('submit',async(e)=>{
  ...
  try{
    ...
    if(d.ok&&d.token){localStorage.setItem('token',d.token);location.href='/admin';}
...
.header a{color:#fff;text-decoration:none;opacity:0.85;font-size:0.9em}
.header a:hover{opacity:1}
.logout{position:absolute;right:24px;top:20px;background:rgba(255,255,255,0.2);padding:6px 14px;border-radius:6px;color:#fff;text-decoration:none;font-size:0.85em}
...
@media(max-width:600px){.header nav{gap:10px}.header a{font-size:0.8em}.logout{right:10px;top:16px;padding:4px 10px}}</style></head>
<body>
<div class="header"><h1>🤖 ${CONFIG.name}</h1><nav><a href="#" onclick="showTab('stats');return false">📊 统计</a><a href="#" onclick="showTab('adapters');return false">🔌 适配器</a><a href="#" onclick="showTab('plugins');return false">🔧 插件</a><a href="#" onclick="showTab('messages');return false">💬 消息</a><a href="#" onclick="showTab('logs');return false">📝 日志</a><a href="#" onclick="showTab('database');return false">🗄️ 数据库</a></nav><a href="/" class="logout" onclick="logout();return false">退出</a></div>
<div class="container">
...
async function loadStats(){
  ...
  document.getElementById('statsGrid').innerHTML=
    '<div class="stat-card"><div class="stat-value">'+(s.totalMsgs||0)+'</div><div class="stat-label">消息总数</div></div>'+
    '<div class="stat-card"><div class="stat-value">'+(s.totalUsers||0)+'</div><div class="stat-label">用户数量</div></div>'+
    '<div class="stat-card"><div class="stat-value">'+(s.activePlugins||0)+'/'+(s.totalPlugins||0)+'</div><div class="stat-label">活跃插件</div></div>'+
    '<div class="stat-card"><div class="stat-value">'+(s.activeAdapters||0)+'/'+(s.totalAdapters||0)+'</div><div class="stat-label">活跃适配器</div></div>'+
    '<div class="stat-card"><div class="stat-value">'+(s.sessions||0)+'</div><div class="stat-label">在线会话</div></div>'+
    '<div class="stat-card"><div class="stat-value">'+(s.uptime||'-')+'</div><div class="stat-label">运行时间</div></div>'+
    '<div class="stat-card"><div class="stat-value">'+(s.memory?.rss||'-')+'</div><div class="stat-label">内存使用</div></div>'+
    '<div class="stat-card"><div class="stat-value">'+(s.node||'-')+'</div><div class="stat-label">Node版本</div></div>';
}
async function loadAdapters(){
  const a=await api('/api/adapters');
  let html='<table><thead><tr><th>名称</th><th>类型</th><th>描述</th><th>状态</th><th>操作</th></tr></thead><tbody>';
...
async function loadPlugins(){
  ...
  let html='<table><thead><tr><th>名称</th><th>描述</th><th>状态</th><th>操作</th></tr></thead><tbody>';
...
async function loadMsgs(){
  ...
  if(m.length===0){document.getElementById('msgList').innerHTML='<div class="empty">暂无消息</div>';return;}
  let html='';
  for(const x of m){html+='<div class="msg-item"><div class="msg-time">'+x.time+'</div><div class="msg-content">'+(x.text||JSON.stringify(x))+'</div></div>';}
  document.getElementById('msgList').innerHTML=html;
}
async function loadLogs(){
  const l=await api('/api/logs');
  document.getElementById('logContent').textContent=l.length?l.map(x=>'['+x.level+'] '+x.msg).join('\n'):'暂无日志';
}
...
async function clearMsgs(){if(!confirm('确定清空所有消息?'))return;await api('/api/messages/clear',{method:'POST'});loadMsgs();loadStats();}
...
async function savePlugin(){
  ...
  await api('/api/plugins/save/'+n,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:c})});
...
async function dbGet(){
  const t=document.getElementById('dbTable').value,k=document.getElementById('dbKey').value;
...
async function dbSet(){
  const t=document.getElementById('dbTable').value,k=document.getElementById('dbKey').value;
  if(!t||!k){alert('请输入表名和键名');return;}
  const v=prompt('请输入值:');if(v===null)return;
  await api('/api/db/set/'+t+'/'+k,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({value:v})});
  alert('设置成功');dbGet();
}
loadStats();
</script></body></html>`)}
